...
public class PlayerMovement : MonoBehaviour // Player 组件可能在父级或子级，故不使用 RequireComponent
{
    ...
    private void AssignObjects()
    {
        ...
        player = GetComponent<Player_PropertyManager>() ?? GetComponentInParent<Player_PropertyManager>() ?? GetComponentInChildren<Player_PropertyManager>();
        if (player == null)
        {
    ...
    void Judege_PlyaerCurrentState_IdleOrRun()
    {
        ...
        if (!isTouching||isTouching && moveDirection == Vector2.zero)
        {
            ...
            player.CurrentState = Player_PropertyManager.PlayerState.Idle;
        }
        //如果玩家有触摸而且移动向量不为零，则切换到 Run 状态
        if (isTouching && moveDirection != Vector2.zero)
        {
            player.CurrentState = Player_PropertyManager.PlayerState.Run;
        }
    }
    ...
    void MovePlayer()
    {
        ...
        if ( player.CurrentState != Player_PropertyManager.PlayerState.Attack && player.CurrentState != Player_PropertyManager.PlayerState.Shoot)
        {
            // 简单的左右翻转
