...
public class PlayerMovement : MonoBehaviour // Player 组件可能在父级或子级，故不使用 RequireComponent
{
    ...
    private void AssignObjects()
    {
        ...
        playerState = GetComponent<PlayerStatus>() ?? GetComponentInParent<PlayerStatus>() ?? GetComponentInChildren<PlayerStatus>();
        if (playerState == null)
        {
    ...
    void Judege_PlyaerCurrentState_IdleOrRun()
    {
        ...
        if (!isTouching||isTouching && moveDirection == Vector2.zero)
        {
            ...
            playerState.CurrentState = PlayerStatus.PlayerState.Idle;
        }
        //如果玩家有触摸而且移动向量不为零，则切换到 Run 状态
        if (isTouching && moveDirection != Vector2.zero)
        {
            playerState.CurrentState = PlayerStatus.PlayerState.Run;
        }
    }
    ...
    void MovePlayer()
    {
        ...
        if ( playerState.CurrentState != PlayerStatus.PlayerState.Attack && playerState.CurrentState != PlayerStatus.PlayerState.Shoot)
        {
            // 简单的左右翻转
