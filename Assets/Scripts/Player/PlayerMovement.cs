...
public class PlayerMovement : MonoBehaviour // Player 组件可能在父级或子级，故不使用 RequireComponent
{
    ...
    private Player_PropertyManager playerState;
    ...
    private void AssignObjects()
    {
        ...
        playerState = GetComponent<Player_PropertyManager>() ?? GetComponentInParent<Player_PropertyManager>() ?? GetComponentInChildren<Player_PropertyManager>();
        if (playerState == null)
        {
            Debug.LogError("PlayerMovement 无法找到 Player_PropertyManager 组件（当前对象或其父/子）。请确保存在附带 Player_PropertyManager 脚本的物体。");
            enabled = false;
            return;
        }
    ...
    void Update()
    {
        ...
        // 判断Player的当前状态
        battleLogic.Judge_PlyaerCurrentState_AttackOrShoot();  // Attack 或 Shoot 状态
        Judege_PlyaerCurrentState_IdleOrRun();// Idle 或 Run 状态
        

        //判断玩家当前状态并播放对应动画
        switch (playerState.CurrentState)
        {
            case Player_PropertyManager.PlayerState.Attack:
                battleLogic.PlayMeleeAttackAnimation();
                break;
            case Player_PropertyManager.PlayerState.Shoot:
                battleLogic.PlayShootAnimation();
                break;
            case Player_PropertyManager.PlayerState.Idle:
                animator.SetBool("isRun", false);
                break;
            case Player_PropertyManager.PlayerState.Run:
    ...
    // 根据当前输入和状态判断是否切换到 Idle 或 Run 状态
    void Judege_PlyaerCurrentState_IdleOrRun()
    {
        //如果玩家不触摸 或者 有触摸而且移动向量为零，则切换到 Idle 状态
        if (!isTouching||isTouching && moveDirection == Vector2.zero)
        {
            rb2D.velocity = Vector2.zero;//确保完全静止
            playerState.CurrentState = Player_PropertyManager.PlayerState.Idle;
        }
        //如果玩家有触摸而且移动向量不为零，则切换到 Run 状态
        if (isTouching && moveDirection != Vector2.zero)
        {
            playerState.CurrentState = Player_PropertyManager.PlayerState.Run;
        }
    }
    ...
    void MovePlayer()
    {
        ...
        if ( playerState.CurrentState != Player_PropertyManager.PlayerState.Attack && playerState.CurrentState != Player_PropertyManager.PlayerState.Shoot)
        {
            // 简单的左右翻转
